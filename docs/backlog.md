...
- [ ] **A fully public, re-runnable FAIL → fix loop.** The single highest-leverage credibility gap (external review, 2026-07-06): every field number is testimony against private repos, and the worked example is curated backwards-from-failure. Build one small public fixture app (in-repo under `examples/`, with its own test setup) plus a one-command script that reproduces a complete live cycle a reader can re-run themselves: red gate → cold-review FAIL → bounded remediation → STRONG_RED → PASS. This converts one instance of the story from labeled testimony into mechanical proof.
